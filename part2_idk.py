...
def find_longest_path_setup(G, dpth):
    ...
    if size <= 30:
        ...
        c = 5
    edges, nodes = [], []
    while len(edges) < k or len(nodes) < c:
        # for n in nodes:
        #     G.remove_node(n)
        while len(edges) < k:
            dpth = min(k - len(edges), dpth)
            curr = find_longest_path(G, k, dpth, True, size)
            if not curr:
                break
            if type(curr) == tuple:
                edges.append(curr)
        ...
        print("before", edges, nodes)
        edges, nodes = set_cover_nodes(edges, c, size, G)
        print("after", edges, nodes)
    ...
    print("current:", curr_length)
    return [edges, nodes]


def find_longest_path(G, k, depth, selection, size):
    # print(depth)
    try:
    ...
    if k > 0:
        for i in range(len(shortest_path) - 1):
            ...
            curr_path_length = find_longest_path(G, k - 1, depth - 1, False, size)
